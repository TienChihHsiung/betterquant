...
class FeeInfoCache {
 ...
 public:
  ...
  FeeInfoSPtr getFeeInfo(const OrderInfo* orderInfo) const;
  FeeInfoSPtr getFeeInfo(AcctId acctId, const std::string& side,
                         const std::string& marketCode,
                         const std::string& symbolType,
                         const std::string& symbolCode) const;

  Decimal calcFee(const OrderInfo* orderInfo, Decimal dealAmt) const;
